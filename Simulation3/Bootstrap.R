...
gamma2_bootstrap <- matrix(NA, nr = Nbts, nc = 2)
...
for(iterboots in 1:Nbts){
  ...
  BootstrapEM <- Bootstrap_optimization(betaEstimate_initBootstrap, btemp_initBootstrap, sigma_beta_initBootstrap, sigma3_initBootstrap, gamma_initialBootstrap, lambda0_initialBootstrap, statusboots, timeboots, indexboots, dboots, Y_boots, xi_boots,I, leftInterval_temp, rightInterval_temp)
  ...
  gamma2_bootstrap[iterboots,] <- BootstrapEM$gamma_temp
  print(iterboots)
}
...
sd_gamma2 <- apply(gamma2_bootstrap, 2, sd) 
