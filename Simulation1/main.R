...
filename_gamma = './results/gamma'
filename_alpha = './results/alpha'
...
filename_gammasd = './results/gammasd'
filename_alphasd = './results/alphasd'
...
DoBootstrap = 1
alpha_store <- matrix(NA, nr = Nsimulations, nc = 16)
...
gamma_store <- matrix(NA, nr = Nsimulations, nc = 19)
Niters_store <- rep(NA, Nsimulations)

alphasd_store <- matrix(NA, nr = Nsimulations, nc = 16)
...
#gamma1_store <- matrix(NA, nr = Nsimulations, nc = ncol(Z))
gammasd_store <- matrix(NA, nr = Nsimulations, nc = 19)
ifAppend=FALSE;
for(isim in 1:Nsimulations){
  ...
  if(DoBootstrap == 1){
    ...
    gammasd_store[isim,] <- sd_gamma
    alphasd_store[isim,] <- sd_alpha
    ...
    write.table(matrix(round(sd_gamma, digits = 6),nr=1), file = paste(filename_gammasd,".txt",sep=""), append = ifAppend, row.names = FALSE,
                col.names = FALSE);
    write.table(matrix(round(sd_alpha, digits = 6),nr=1), file = paste(filename_alphasd,".txt",sep=""), append = ifAppend, row.names = FALSE,
                col.names = FALSE);
  }
  ...
  write.table(matrix(round(gamma_temp, digits = 6),nr=1), file = paste(filename_gamma,".txt",sep=""), append = ifAppend, row.names = FALSE,
              col.names = FALSE);
  
    write.table(matrix(round(alpha_temp, digits = 6),nr=1), file = paste(filename_alpha,".txt",sep=""), append = ifAppend, row.names = FALSE,
              col.names = FALSE);
